		...
		for _, ciliumPod := range ct.CiliumPods() {
			...
			for frontendEntry, backendEntry := range resMap {
				...
				if len(backendEntry) > 0 {
					lbMap[frontendEntry] = append(lbMap[frontendEntry], backendEntry...)
				}
			}
			// Check for frontend and backend mapping in the parsed BPF LB map.
			for _, backend := range backends {
				found := false
				for fe, beStr := range lbMap {
					if fe != frontend {
						continue
					}
					for _, be := range beStr {
				...
				if !found {
					return fmt.Errorf("frontend [%s] backend [%s] mapping not found in BPF LB map [%s] %+v", frontend, backend, ciliumPod.Pod.Name, lbMap)
				}
			}
		}
